...
public class MyOffers {
    public static void main(String[] args) {
        ...
        offer1.setInfo("VA", "Amazin Inc", "SDET", 110000, true,true,true,true);
        ...
        Offer [] myOffers = {offer1, offer2, offer3, offer4, offer5};  // first step is to create an Array
        ArrayList<Offer> fullTimeOffers = new ArrayList<>(Arrays.asList(myOffers)); // second step is converting it to an ArrayList
        fullTimeOffers.removeIf(p-> !p.isFullTime); // removes if the offer is NOT full time
        System.out.println(fullTimeOffers.size()); // 4  number of full time offers
        System.out.println("fullTimeOffers = " + fullTimeOffers);
        System.out.println("-------------------------------------------------------------------------------");

        ArrayList<Offer> localOffers = new ArrayList<>(Arrays.asList(myOffers));
        localOffers.removeIf(p-> !(p.location.equals("VA") || p.location.equals("WA"))); // removes the offer if the offer is not from local area
